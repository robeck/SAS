...
3.明确double_sorted仅仅包括rsj_resdiual_rsk和rsk_rsj,其他参数不参与计算
4.rsj和rsk的回归残差resdiual已经参与corss_section等计算，随时可以调用
****************************************
...
/*这里创建的数据集为分组5*5组时*/
data C.w_11_252_&v C.w_12_252_&v C.w_13_252_&v C.w_14_252_&v C.w_15_252_&v C.w_21_252_&v C.w_22_252_&v C.w_23_252_&v C.w_24_252_&v C.w_25_252_&v C.w_31_252_&v C.w_32_252_&v C.w_33_252_&v
C.w_34_252_&v C.w_35_252_&v C.w_41_252_&v C.w_42_252_&v C.w_43_252_&v C.w_44_252_&v C.w_45_252_&v C.w_51_252_&v C.w_52_252_&v C.w_53_252_&v C.w_54_252_&v C.w_55_252_&v;
run;
...
%if &j=1 %then %do;
	...
	%if &m=1 %then %do;
	data C.w_11_252_&v;
	set C.w_11_252_&v &w&m;
	run;
	%end;

	%if &m=2 %then %do;
	data C.w_12_252_&v;
	set C.w_12_252_&v &w&m;
	run;
	%end;

	%if &m=3 %then %do;
	data C.w_13_252_&v;
	set C.w_13_252_&v &w&m;
	run;
	%end;

	%if &m=4 %then %do;
	data C.w_14_252_&v;
	set C.w_14_252_&v &w&m;
	run;
	%end;

	%if &m=5 %then %do;
	data C.w_15_252_&v;
	set C.w_15_252_&v &w&m;
	run;
	%end;
...
%if &j=2 %then %do;
	...
	%if &m=1 %then %do;
	data C.w_21_252_&v;
	set C.w_21_252_&v &w&m;
	run;
	%end;

	%if &m=2 %then %do;
	data C.w_22_252_&v;
	set C.w_22_252_&v &w&m;
	run;
	%end;

	%if &m=3 %then %do;
	data C.w_23_252_&v;
	set C.w_23_252_&v &w&m;
	run;
	%end;

	%if &m=4 %then %do;
	data C.w_24_252_&v;
	set C.w_24_252_&v &w&m;
	run;
	%end;

	%if &m=5 %then %do;
	data C.w_25_252_&v;
	set C.w_25_252_&v &w&m;
	run;
	%end;
...
%if &j=3 %then %do;
	...
	%if &m=1 %then %do;
	data C.w_31_252_&v;
	set C.w_31_252_&v &w&m;
	run;
	%end;

	%if &m=2 %then %do;
	data C.w_32_252_&v;
	set C.w_32_252_&v &w&m;
	run;
	%end;

	%if &m=3 %then %do;
	data C.w_33_252_&v;
	set C.w_33_252_&v &w&m;
	run;
	%end;

	%if &m=4 %then %do;
	data C.w_34_252_&v;
	set C.w_34_252_&v &w&m;
	run;
	%end;

	%if &m=5 %then %do;
	data C.w_35_252_&v;
	set C.w_35_252_&v &w&m;
	run;
	%end;
...
%if &m=1 %then %do;
	data C.w_41_252_&v;
	set C.w_41_252_&v &w&m;
	run;
	%end;

	%if &m=2 %then %do;
	data C.w_42_252_&v;
	set C.w_42_252_&v &w&m;
	run;
	%end;

	%if &m=3 %then %do;
	data C.w_43_252_&v;
	set C.w_43_252_&v &w&m;
	run;
	%end;

	%if &m=4 %then %do;
	data C.w_44_252_&v;
	set C.w_44_252_&v &w&m;
	run;
	%end;

	%if &m=5 %then %do;
	data C.w_45_252_&v;
	set C.w_45_252_&v &w&m;
	run;
	%end;
...
%if &m=1 %then %do;
	data C.w_51_252_&v;
	set C.w_51_252_&v &w&m;
	run;
	%end;

	%if &m=2 %then %do;
	data C.w_52_252_&v;
	set C.w_52_252_&v &w&m;
	run;
	%end;

	%if &m=3 %then %do;
	data C.w_53_252_&v;
	set C.w_53_252_&v &w&m;
	run;
	%end;

	%if &m=4 %then %do;
	data C.w_54_252_&v;
	set C.w_54_252_&v &w&m;
	run;
	%end;

	%if &m=5 %then %do;
	data C.w_55_252_&v;
	set C.w_55_252_&v &w&m;
	run;
	%end;
...
%let b=5;
/*这里创建的数据集为分组5组时*/
data D.Q_252_&v&t D.Q_252_&v&y D.Q_252_&v&g D.Q_252_&v&h D.Q_252_&v&b;
run;
...
data E.reg_multiple_252(keep=rsj rkt rsk rovl);
set E.reg_multiple_252 est;
run;
...
data E.reg_rsj_252(keep=rsj);
set E.reg_rsj_252 estrsj;
run;
...
data E.reg_rsk_252(keep=rsk);
set E.reg_rsk_252 estrsk;
run;
...
data E.reg_rkt_252(keep=rkt);
set E.reg_rkt_252 estrkt;
run;
...
data E.reg_rovl_252(keep=rovl);
set E.reg_rovl_252 estrovl;
run;
